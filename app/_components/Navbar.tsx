    ...
    >
      <div className="flex justify-between items-center px-gutter max-w-[1600px] mx-auto">
          ...
          {navLinks.map((link) => {
            if (link.subLinks) {
              ...
              return (
                <div key={link.label} className="relative group">
                  <button
                    className={`font-medium transition-all duration-300 text-base pb-1 border-b-2 flex items-center gap-1 ${isActive
                  ...
                  <div className="absolute top-full left-0 mt-2 w-48 bg-surface border border-outline-variant rounded-xl shadow-ambient-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 overflow-hidden">
                    {link.subLinks.map((sub) => (
                      <Link
                        key={sub.href}
                        href={sub.href}
                        className={`block px-4 py-3 text-sm transition-colors ${pathname === sub.href
            ...
            return (
              <Link
                key={link.href!}
                href={link.href!}
                className={`font-medium transition-all duration-300 text-base pb-1 border-b-2 ${isActive
        ...
        <div className="flex items-center gap-sm">
          <Link
            href="/admin/login"
            className={`hidden md:inline-flex text-sm font-medium px-md py-xs rounded-full hover:shadow-xl hover:scale-105 transition-all duration-300 ${isTransparent
