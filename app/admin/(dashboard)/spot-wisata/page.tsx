...
export default function AdminSpotWisataPage() {
  ...
  function openCreate() { setForm(emptyForm); setEditingId(null); setShowForm(true); setMessage(null) }
  function openEdit(item: SpotWisata) { setForm(item); setEditingId(item.id); setShowForm(true); setMessage(null) }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const supabase = createClient()
      const payload = {
        nama_lokasi: form.nama_lokasi, kategori: form.kategori,
        latitude: Number(form.latitude), longitude: Number(form.longitude),
        deskripsi: form.deskripsi, gambar_url: form.gambar_url,
        jam_operasional: form.jam_operasional, status: form.status,
      }
      if (editingId) {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-lg gap-md">
        <div>
          <h1 className="text-3xl font-bold text-on-surface">Kelola Spot Wisata</h1>
          <p className="text-on-surface-variant text-base mt-xs">Atur dan kelola titik-titik lokasi ekowisata.</p>
        </div>
          ...
          <div className="bg-surface-container-lowest rounded-3xl p-xl w-full max-w-[42rem] max-h-[90vh] overflow-y-auto shadow-ambient-lg border border-outline-variant animate-fade-in-up">
            ...
            <form onSubmit={handleSubmit} className="space-y-md">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-xs">Nama Lokasi *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-xs">Kategori *</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-xs">Latitude *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-xs">Longitude *</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-xs">Jam Operasional</label>
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-xs">Deskripsi *</label>
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-xs">URL Gambar</label>
                <input value={form.gambar_url || ''} onChange={(e) => setForm({ ...form, gambar_url: e.target.value })} className="w-full bg-surface-container-low border border-outline-variant rounded-xl px-md py-3 focus:ring-2 focus:ring-primary text-on-surface" placeholder="https://example.com/image.jpg" />
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-lg">
        {loading ? (
          ...
          <div className="col-span-full text-center py-xl">
            ...
            <p className="text-on-surface-variant text-lg mt-md">Belum ada spot wisata.</p>
          </div>
        ) : (
          items.map((item) => (
            <div key={item.id} className="bg-surface-container-lowest rounded-3xl border border-outline-variant shadow-sm overflow-hidden hover:shadow-ambient-hover transition-all group">
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-3 right-3">
                  <span className={`text-xs px-sm py-1 rounded-full font-bold ${item.status === 'published' ? 'bg-tertiary text-on-tertiary' : 'bg-outline text-on-primary'}`}>
                    {item.status === 'published' ? 'PUBLISHED' : 'DRAFT'}
                  </span>
                </div>
              </div>
              <div className="p-md">
                <h3 className="font-semibold text-on-surface text-lg">{item.nama_lokasi}</h3>
                <p className="text-sm text-on-surface-variant mt-1 line-clamp-2">{item.deskripsi}</p>
                ...
                <div className="flex items-center justify-between mt-md">
                  <span className="text-xs bg-surface-container-high px-sm py-1 rounded-full text-on-surface-variant">{item.kategori}</span>
